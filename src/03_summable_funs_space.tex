...
\begin{gather*}
    ...
    1 \leq p < + \infty \quad ||x - x^{(m)}||_p = \left( \sum^\infty_{k=m+1} \frac{1}{2^{k+p}} \right)^{\frac{1}{p}} \underset{m \to \infty}{\longrightarrow} 0
\end{gather*}
...
\end{gather*}
Любой элемент из $c_0$ является пределом элементов из $F$ по норме $\norm{\cdot}_\infty$.
